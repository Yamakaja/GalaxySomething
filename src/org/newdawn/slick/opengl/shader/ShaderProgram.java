...
public class ShaderProgram {
    ...
    public ShaderProgram(String vertexShaderSource, String fragShaderSource) throws SlickException {
    	...
    	vertexHandle = compileShader(VERTEX_SHADER, vertexShaderSource);
    	fragmentHandle = compileShader(FRAGMENT_SHADER, fragShaderSource);
	...
	protected void attachShaders() {
		ARBShaderObjects.glAttachObjectARB(getID(), vertexHandle);
		ARBShaderObjects.glAttachObjectARB(getID(), fragmentHandle);
	}
	...
	protected void linkProgram() throws SlickException {
        ...
        int comp = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_LINK_STATUS);
        int len = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_INFO_LOG_LENGTH);
		String err = ARBShaderObjects.glGetInfoLogARB(programHandle, len);
	...
	public void releaseShaders() {
		unbind();
		if (vertexHandle!=0) {
			ARBShaderObjects.glDetachObjectARB(getID(), vertexHandle);
			ARBShaderObjects.glDeleteObjectARB(vertexHandle);
			vertexHandle = 0;
		}
		if (fragmentHandle!=0) {
			ARBShaderObjects.glDetachObjectARB(getID(), fragmentHandle);
	...
	private void fetchUniforms() {
		int len = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_ACTIVE_UNIFORMS);
		//max length of all uniforms stored in program
		int strLen = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_ACTIVE_UNIFORM_MAX_LENGTH);
		
		for (int i=0; i<len; i++) {
			String name = ARBShaderObjects.glGetActiveUniformARB(programHandle, i, strLen);
			int id = ARBShaderObjects.glGetUniformLocationARB(programHandle, name);
			uniforms.put(name, id);
		}
	}
	
	private void fetchAttributes() {
		int len = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_ACTIVE_ATTRIBUTES);
		//max length of all uniforms stored in program
		int strLen = ARBShaderObjects.glGetObjectParameteriARB(programHandle, GL20.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH); 
		for (int i=0; i<len; i++) {
			String name = ARBVertexShader.glGetActiveAttribARB(programHandle, i, strLen);
			int id = ARBVertexShader.glGetAttribLocationARB(programHandle, name);
			uniforms.put(name, id);
		}
	}
	...
	public int getUniformID(String name) {
		...
		if (location!=-1)
			return location;
		location = ARBShaderObjects.glGetUniformLocationARB(programHandle, name);
	...
	public int getAttributeID(String name) {
		...
		if (location!=-1)
			return location;
		location = ARBVertexShader.glGetAttribLocationARB(programHandle, name);
		if (location == -1 && strict)
...
//	}
	...
	public boolean getUniform(String name, FloatBuffer buf) {
		...
		if (id==-1) return false;
		ARBShaderObjects.glGetUniformARB(programHandle, id, buf);
		return true;
	}
