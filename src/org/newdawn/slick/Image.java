...
import org.newdawn.slick.opengl.TextureData;
...
public class Image implements Renderable {
	...
	public static final BlendMode BLEND_MODE_ONLY_SOURCE = new BlendMode(GL11.GL_ONE, GL11.GL_ZERO);
    ...
    /** The BlendMode (glBlendFunc) this image is rendered with */
    protected BlendMode blendMode = null;
    
    /** True if the image should be flipped vertically. Note that this is never even used. */
    private boolean flipped;
    /** The transparent colour set if any. Note that this is never even used. */
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public Image(InputStream in) throws SlickException {
		this(in, in.toString(), false);
	}
	...
	Image(ImageBuffer buffer, int filter) {
		this((TextureData) buffer, filter);
        TextureImpl.bindNone();
	}
	 ...
	 */
	public Image(TextureData data, int f) {
		try {
			this.filter = f == FILTER_LINEAR ? SGL.GL_LINEAR : SGL.GL_NEAREST;
	...
	public void draw(float x, float y, float x2, float y2, float srcx, float srcy, float srcx2, float srcy2, Color filter) {
        ...
        if (blendMode != null) {
        	...
        	Logger.print("blendMode is null!!!!!");
        }
...
//		GL.glEnd();
	...
	/**
	 * Set the mode used to blend this image when drawn
	 */
	public Image setBlendMode(BlendMode mode) {
		this.blendMode = mode;
		return this;
	}
	
	/**
	 * Get the mode used to blend this image when drawn
	 */
	...
	public static class BlendMode {
		...
		private ArrayList<BlendMode> previousBlendModes;
		
		public BlendMode(int src, int dst) {
			this.src = src;
			this.dst = dst;
			previousBlendModes = new ArrayList<>(3);
		}
		
		protected BlendMode(int src, int dst, boolean dontCreateArrayList) {
			this.src = src;
			this.dst = dst;
		}
		
		public void bind(SGL GL) {
			previousBlendModes.add(new BlendMode(GL11.glGetInteger(GL11.GL_BLEND_SRC), GL11.glGetInteger(GL11.GL_BLEND_DST), false));
			GL.glBlendFunc(src, dst);
		}
		
		public void unbind(SGL GL) {
			BlendMode previous = previousBlendModes.remove(previousBlendModes.size() - 1);
			GL.glBlendFunc(previous.src, previous.dst);
		}
	}
}
